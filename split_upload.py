...
Example (fast, default - 100MB chunks, 8 download connections, 6 upload workers):
    python3 multi_upload.py "https://example.com/file.zip"

Example (push it even further):
    python3 multi_upload.py "https://example.com/file.zip" 50 8 0.2 12
"""
import sys
import os
import time
...
def main():
    if len(sys.argv) < 2:
        ...
        print("Defaults: 100MB chunks, 6 upload workers, 0.3s min delay, 8 download connections.")
        sys.exit(1)

    url = sys.argv[1]
    chunk_size_mb = int(sys.argv[2]) if len(sys.argv) > 2 else 100
    upload_workers = int(sys.argv[3]) if len(sys.argv) > 3 else 6
    download_workers = int(sys.argv[5]) if len(sys.argv) > 5 else 8
    global MIN_SECONDS_BETWEEN_REQUESTS
    MIN_SECONDS_BETWEEN_REQUESTS = float(sys.argv[4]) if len(sys.argv) > 4 else 0.3
